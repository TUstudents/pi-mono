...
- **Agent class moved to `@cargo-cult/pi-agent-core`**: The `Agent` class, `AgentState`, and related types are no longer exported from this package. Import them from `@cargo-cult/pi-agent-core` instead.
...
- **`AppMessage` renamed to `AgentMessage`**: Now imported from `@cargo-cult/pi-agent-core`. Custom message types use declaration merging on `CustomAgentMessages` interface.
...
- **`CustomMessages` interface removed**: Use declaration merging on `CustomAgentMessages` from `@cargo-cult/pi-agent-core` instead.

- **`agent.appendMessage()` removed**: Use `agent.queueMessage()` instead.

- **Agent event types changed**: `AgentInterface` now handles new event types from `@cargo-cult/pi-agent-core`: `message_start`, `message_end`, `message_update`, `turn_start`, `turn_end`, `agent_start`, `agent_end`.
...
- `Agent` class (moved to `@cargo-cult/pi-agent-core`)
...
**Before (0.30.x):**
```typescript
import { Agent, ProviderTransport, type AppMessage } from '@cargo-cult/pi-web-ui';
...
**After:**
```typescript
import { Agent, type AgentMessage } from '@cargo-cult/pi-agent-core';
import { defaultConvertToLlm } from '@cargo-cult/pi-web-ui';
...
declare module "@cargo-cult/pi-web-ui" {
